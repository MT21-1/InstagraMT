...
export default function PostPage(){
    ...
    const [commentCountVisible, setCommentCountVisible] = useState(4);
                    ...
                    <div className="postDiv">
                        ...
                        <Popup visible={visible3} onClose={() => setVisible3(false)} >
                            <div className="deleteModal">
                                ...
                                <input type="text" autoComplete="off" onChange={(e) =>{setNewCaption(e.target.value)}}/>
                                <button id="confirm" onClick={editPst}>Update</button>
                            </div>
                                    
                        </Popup>      
                        <div className="postLeftDiv">
                            {/* Profile */}
                            <div className="postProfileDiv">
                            
                                <a href={"/profile/" + username} id="username">
                                    <img src={userProfile} alt="" />
                                    <span>{username}</span>
                                </a>
                                {(userId == currUser.id)?<Popup2 trigger={
                                    <button id="postSettingButton"> 
                                        ...
                                        <div className="popUpPost">
                                            <button id="editButton" onClick={()=>setVisible3(true)}>Edit</button>
                            ...
                            {(image.length != 0 && image != undefined && image != null) ?(
                                <div className="pictureDiv">    
                                    ...
                                    {image.map((content, i)=>{
                                        return ((i == idx)?((content.type == "image")?<img className="displayed" id= {(i).toString()} src={content.path} alt="" key={content.id}/>: <video className="displayed" id = {(i).toString()}src={content.path} key = {content.id} controls preload="auto"></video>):null)
                                    })}
                                ...
                                (
                                    <div className="pictureDiv">    
                                        {commentArray.map((content, i)=>{
                                            console.log(content)
                                            return((i <= commentCountVisible)?(<Comment comment_id={content.id} comment_value={content.comment} comment_user_id = {content.user_id} comment_header_id= {-1} comment_replies = {content.replies}/>): null) 
                                        })}
                                    </div>
                                ):
                                    (loading)?loadingBtn: "No Comments Available"
                                }
                                {(commentCountVisible < commentArray.length-1)? 
                                <a href="#" className="viewMoreComments" onClick={()=>((commentCountVisible + 5 > commentArray.length-1)?setCommentCountVisible(commentArray.length-1):setCommentCountVisible(commentCountVisible+5))}> View more</a>:null}
